...
class Table(container.Container):
	...
	def kill(self):
		...
		for aRow in self._rows:
			for aCell in aRow:
				if isinstance(aCell, dict) and aCell:
					aCell['widget'].kill();
		self._rows= [];
	...
	def td(self, w, col=None, row=None, colspan=1, rowspan=1, **params):
		...
		"""
		tempTd= _Table_td(w, **params);
		Table.add(self,tempTd, col=col, row=row, colspan=colspan, rowspan=rowspan)
...
class _Table_td(container.Container):
	def __init__(self,widget,**params):#hexpand=0,vexpand=0,
		...
		widget._table_td = weakref.ref(self)
		widget.container= weakref.ref(self);
		self.add(widget,0,0)
	
	def kill(self):
		if self.widget:
			self.widget._table_td = None;
		container.Container.kill(self);
		if self.widget:
			self.widget.kill();
			self.widget= None;
