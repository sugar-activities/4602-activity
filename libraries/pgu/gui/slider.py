...
class _slider(widget.Widget):
	...
	def event(self,e):
		...
		if e.type == ENTER: self.repaint()
			...
			if 1 in e.buttons and self.container and self.container().myfocus and self.container().myfocus() is self:
				if self.grab != None:
		...
		self.pcls = ""
		if self.container and self.container().myhover and self.container().myhover() is self: self.pcls = "hover"
		if (self.container and self.container().myfocus and self.container().myfocus() is self and 1 in pygame.mouse.get_pressed()): self.pcls = "down"
