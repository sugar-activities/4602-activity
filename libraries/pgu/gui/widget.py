...
class Widget:
	...
	def focus(self):
		...
		"""
		if self.hasContainer(): 
			unfocused = not self.container().myfocus
			notAlreadyFocused= self.container().myfocus and self.container().myfocus() != self
			if unfocused or notAlreadyFocused:  ## by Gal Koren
	...
	def blur(self): 
		...
		<pre>Widget.blur()</pre>
		"""
		if self.hasContainer(): self.container().blur(self)
	def open(self):
		"""Open this Widget as a modal dialog.
		
		<pre>Widget.open()</pre>
		"""
		if self.hasContainer(): self.container().open(self)
	def close(self): 
		"""Close this Widget (if it is a modal dialog.)
		
		<pre>Widget.close()</pre>
		"""
		if self.hasContainer(): self.container().close(self)
	def resize(self,width=None,height=None):
	...
	def hasContainer(self):
		return getattr(self,'container',None) != None and self.container() != None;
	...
	def repaint(self): 
		...
		"""
		if self.hasContainer(): self.container().repaint(self)
	def repaintall(self):
		"""Request a repaint of all Widgets.
		
		<pre>Widget.repaintall()</pre>
		"""
		if self.hasContainer(): self.container().repaintall()
	def reupdate(self): 
		"""Request a reupdate of this Widget
		
		<pre>Widget.reupdate()</pre>
		"""
		if self.hasContainer(): self.container().reupdate(self)
	def next(self): 
		...
		<pre>Widget.next()</pre>
		"""
		if self.hasContainer(): self.container().next(self)
	def previous(self): 
		...
		if self.hasContainer(): self.container().previous(self)
	...
	def connect(self,code,fnc,*values):
		...
		"""
		self.connects[code] = {'fnc':weakProxy.Proxy(fnc),'values':values}
	def connectWeakly(self,code,_fnc,*values):
		self.connects[code] = {'fnc':weakProxy.Proxy(_fnc), 'values':values};
	...
	def send(self,code,event=None):
		...
		if code in self.connects:
			...
			fnc = con['fnc']
			if isinstance(fnc, weakProxy.Proxy):
				fnc= fnc.makeCallable();
			if fnc is None:
				return
